...
def getwatchlist(idwatchlist):
    ...
    if skip == 0:
        ...
        # total = db["watchlists"].find_one({"id": idwatchlist})
        ...
        return {"result": cvtJson(watchlist["items"]), "total": len(watchlist["items"])}


def search_watchlist(idwatchlist):
    # skip = request.args.get("skip", default=0, type=int)
    ...
    if len(query) != 0:
        ...
        watchlist = db["watchlists"].find_one({"id": idwatchlist})
        total = db["watchlists"].aggregate(
            [
                {"$match": {"id": str(idwatchlist)}},
                {
...
def getitem_watchlist(idwatchlist, idmovie):
    try:
        item_watchlists = db["watchlists"].find_one(
...
def additem_watchlist(idwatchlist):
    try:
        ...
        if media_type == "movie":
            ...
            if movie != None:
                ...
                if "items" not in item_watchlists:
                    ...
                    if float(seconds) > old_seconds and float(percent) > old_percent:
                        db["watchlists"].find_one_and_update(
                            {"id": idwatchlist},
...
def remove_item_watchlist(idwatchlist):
    try:
        media_id = request.form["media_id"]
        ...
        list = cvtJson(db["watchlists"].find_one({"id": idwatchlist}))
...
def removeall_item_watchlist(idwatchlist):
    try:
        db["watchlists"].find_one_and_update(
            {"id": idwatchlist},
            {"$set": {"items": []}},
        ...
        return {"success": False, "result": "Failed to remove all item from watchlist"}
